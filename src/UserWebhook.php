    ...
    public static function showForUser(User $user)
    {
        ...
        if ($canedit) {
            if (function_exists('renderTwigForm')) {
                $form = [
                    ...
                    'itemtype' => self::class,
                    'content' => [
                        __('Add a webhook to user', 'webhook') => [
                            'visible' => true,
                            'inputs' => [
                                '' => [
                                    'type' => 'hidden',
        ...
        if (function_exists('renderTwigForm')) {
            ...
            foreach ($rows as $data) {
                ...
                $row[] = "<a href='$webhookUrl'>" . Html::entities_deep($data['webhook_name'] ?? __('Unknown')) . "</a>";
                $row[] = Html::entities_deep($data['webhook_url'] ?? '');
                $row[] = Dropdown::getYesNo($data['is_active']);
                $values[] = $row;
                if ($canedit) {
                    $massiveValues[] = 'item[' . __CLASS__ . '][' . $data['id'] . ']';
                }
            }

            renderTwigTemplate('table.twig', [
                'id' => 'TableForUserWebhook' . $rand,
                'fields' => $fields,
                'values' => $values,
                $canedit ? 'massive_action' : '' => $massiveValues,
                'noToolBar' => true,
            ]);
        } else {
            // Legacy table (v1)
